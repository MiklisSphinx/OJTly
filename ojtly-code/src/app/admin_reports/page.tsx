...
export default function AdminReportPage() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden">
      ...
      {/* ============================================ */}
      {/* MAIN CONTENT - fills exact remaining space  */}
      {/* ============================================ */}
      <main className="fixed inset-0 lg:left-[280px] z-10 flex flex-col bg-gradient-to-br from-slate-50 via-white to-blue-50/30">
        
        {/* Header */}
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 shadow-sm shrink-0">
          <div className="h-14 sm:h-16 flex items-center justify-between px-4 sm:px-6 lg:px-8">
            
            <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
              <button 
                onClick={() => setIsSidebarOpen(true)} 
                className="lg:hidden p-2 -ml-1 hover:bg-slate-100 rounded-xl active:scale-95 transition-transform shrink-0"
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] sm:text-xs text-slate-400 hidden sm:block truncate mt-0.5">
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0 ml-2">
              <button className="hidden sm:flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-semibold rounded-lg transition-all active:scale-95">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8">
            ...
            {/* Title & Timeframe */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/50 backdrop-blur-sm rounded-2xl p-4 sm:p-5 border border-slate-100/50">
              <div className="min-w-0">
              ...
              <div className="flex gap-1.5 sm:gap-2 bg-slate-100 p-1 rounded-xl w-fit shrink-0 self-start overflow-x-auto scrollbar-hide -mx-1 px-1">
                {['This Week', 'This Month', 'This Year'].map(time => (
                  <button 
                    key={time} 
                    onClick={() => setActiveTimeframe(time)} 
                    className={`px-3 sm:px-4 py-2 text-xs font-semibold rounded-lg transition-all whitespace-nowrap ${
              ...
              {[
                ...
                >
                  <div className="flex items-start justify-between mb-3 sm:mb-4">
                    ...
                    <span className="text-[9px] sm:text-[10px] font-semibold text-slate-500 bg-slate-100 px-2 sm:px-2.5 py-1 rounded-md leading-relaxed text-right max-w-[90px] sm:max-w-none">
                      {card.change}
                    </span>
                  </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-5 lg:p-6">
                ...
                <div className="space-y-4 sm:space-y-5 lg:space-y-6 pt-2">
                  {bscsData.map((data) => (
                    <div key={data.label} className="group">
                      <div className="flex items-center justify-between mb-1.5 sm:mb-2 gap-2">
                        ...
                        <span className="text-xs sm:text-sm font-bold text-slate-800 shrink-0 ml-auto tabular-nums">
                          {data.value}
                        </span>
                      </div>
              ...
              {/* Donut Chart */}
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-5 lg:p-6 flex flex-col">
                ...
                <div className="flex items-center justify-center relative my-2 sm:my-4 lg:my-6 min-h-[160px] sm:min-h-[200px] lg:min-h-[240px]">
                  <div 
                    className="w-[140px] h-[140px] sm:w-[180px] sm:h-[180px] lg:w-[210px] lg:h-[210px] rounded-full shadow-inner transition-all duration-1000 ease-out"
                    style={{ 
                  ...
                  <div className="absolute inset-[18px] sm:inset-[24px] lg:inset-[28px] bg-white rounded-full flex flex-col items-center justify-center shadow-lg">
                    <span className="text-lg sm:text-2xl lg:text-3xl font-bold text-slate-800 tabular-nums leading-none">185</span>
                    <span className="text-[8px] sm:text-[10px] text-slate-400 uppercase tracking-wider font-medium mt-0.5 sm:mt-1">Total</span>
                  </div>
                </div>
                
                <div className="space-y-2 sm:space-y-2.5 mt-auto pt-3 sm:pt-4 border-t border-slate-100">
                  {accountData.map((item) => (
                    <div key={item.label} className="flex items-center justify-between gap-2 group">
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        ...
                        <span className={`text-[10px] sm:text-[11px] font-semibold px-1.5 sm:px-2 py-0.5 rounded-md min-w-[32px] sm:min-w-[36px] text-center tabular-nums ${item.bgColor} ${item.textColor}`}>
                          {item.value}%
                        </span>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
              ...
              <div className="bg-gradient-to-br from-violet-500 to-purple-600 rounded-2xl p-4 sm:p-5 text-white shadow-xl shadow-purple-500/20">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 sm:p-5 sm:col-span-1 lg:col-span-2">
                <h4 className="font-bold text-sm text-slate-800 mb-3 sm:mb-4">Quick Insights</h4>
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  <div className="text-center p-2.5 sm:p-3 bg-slate-50 rounded-xl">
                    <p className="text-base sm:text-xl font-bold text-slate-800">89%</p>
                    <p className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 leading-tight">Completion</p>
                  </div>
                  <div className="text-center p-2.5 sm:p-3 bg-slate-50 rounded-xl">
                    <p className="text-base sm:text-xl font-bold text-slate-800">15</p>
                    <p className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 leading-tight">Pending</p>
                  </div>
                  <div className="text-center p-2.5 sm:p-3 bg-slate-50 rounded-xl">
                    <p className="text-base sm:text-xl font-bold text-slate-800">4.8</p>
                    <p className="text-[9px] sm:text-[10px] text-slate-500 mt-0.5 leading-tight">Avg Rating</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <style jsx global>{`
        ...
        aside::-webkit-scrollbar-thumb { background: #334155; border-radius: 100px; }
