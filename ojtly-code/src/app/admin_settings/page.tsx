...
export default function AdminSettingsPage() {
  ...
  const Toggle = ({ enabled, onToggle }: { enabled: boolean; onToggle: () => void }) => (
    <button
      type="button"
      onClick={onToggle}
      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 shrink-0 ${enabled ? 'bg-indigo-600' : 'bg-slate-200'}`}
    >
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50/30 font-sans">
      ...
      <div className="fixed top-3 left-3 right-3 sm:left-auto sm:right-4 sm:top-4 sm:max-w-sm z-[100] flex flex-col gap-2 pointer-events-none">
        {toasts.map(toast => (
          <div key={toast.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl shadow-lg border backdrop-blur-sm animate-slide-in pointer-events-auto ${toast.type === 'success' ? 'bg-emerald-50/95 border-emerald-200 text-emerald-800' : 'bg-red-50/95 border-red-200 text-red-800'}`}>
            <span className="text-sm font-bold shrink-0">{toast.type === 'success' ? '✓' : '✕'}</span>
            <p className="text-xs font-medium flex-1 min-w-0">{toast.message}</p>
            <button onClick={() => setToasts(prev => prev.filter(t => t.id !== toast.id))} className="text-slate-400 hover:text-slate-600 shrink-0 ml-2">
      ...
      <aside className={`fixed top-0 left-0 z-50 h-full w-[280px] max-w-[80vw] bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 overflow-y-auto overflow-x-hidden`}>
        <div className="p-5 pb-4">
          <Link href="/" className="flex items-center gap-3" onClick={() => setIsSidebarOpen(false)}>
            ...
            <div className="min-w-0">
              <span className="text-xl font-bold tracking-tight text-white block">OJTly</span>
        ...
        <nav className="px-3 space-y-0.5 pb-20">
          ...
          <Link href="/admin_cpost" className="flex items-center justify-between px-4 py-2.5 text-slate-400 hover:text-white hover:bg-white/5 rounded-xl font-medium transition-all group" onClick={() => setIsSidebarOpen(false)}>
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <svg className="w-[18px] h-[18px] group-hover:text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
              <span className="truncate">Company Posts</span>
            </div>
            <span className="min-w-[20px] h-5 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center px-1.5 transition-colors shrink-0 ml-2">4</span>
          </Link>
              ...
              <div className="p-5 divide-y divide-slate-100">
                <div className="flex items-center justify-between py-3 gap-3">
                  <div className="min-w-0 flex-1 pr-2">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Temporarily disable the platform for students.</p>
                  </div>
                ...
                <div className="flex items-center justify-between py-3 gap-3">
                  <div className="min-w-0 flex-1 pr-2">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Allow BSCS students to create their own accounts.</p>
                  </div>
              ...
              <div className="p-5 divide-y divide-slate-100">
                <div className="flex items-center justify-between py-3 gap-3">
                  <div className="min-w-0 flex-1 pr-2">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Get notified when a company applies.</p>
                  </div>
                ...
                <div className="flex items-center justify-between py-3 gap-3">
                  <div className="min-w-0 flex-1 pr-2">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">Get notified when a student finishes OJT.</p>
                  </div>
